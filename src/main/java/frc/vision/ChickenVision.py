...
class WebcamVideoStream:
    def __init__(self, camera, cameraServer, frameWidth, frameHeight, name="WebcamVideoStream"):
        ...
        #Automatically sets exposure to some value to track tape
        self.manualExposure = 2
        self.webcam = camera
        self.webcam.setExposureManual(self.manualExposure)
...
# 20 exposure
lower_green = np.array([64,32,210])
...
# USB30W04MT calibration data
camera_matrix = np.array([[38.95534134, 0, 489.04029881], [0, 22.24751854, 438.4684845], [0, 0, 1]])
dist_coeff = np.array([-.26895196, .08198318, -.00160494, 0.000387, -.01183715])

obj_points = np.array([[5.5, 0, 0], [5.5, 2, 0], [0, 0, 0], [0, 2, 0], 
                        [-2, 9.75, 0], [-2.97, 11.49, 0], [1.84, 14.16, 0], [2.81, 12.41, 0]], dtype=np.float32)
...
# Calculates distance (in meters) from midline of target
# Assumes robot face is parallel to rocket face
# Returns success, angles, distances
def calculateAnglesAndDistances(image_points):
    # print("Image", len(image_points))
    # print("Object", len(obj_points))
    
    (success, rotation_vector, translation_vector) = cv2.solvePnP(obj_points, image_points, camera_matrix, dist_coeff)
    
    angles = rotationMatrixToEulerAngles(rotation_vector)
    return success, angles, translation_vector
...
def findTape(contours, image, centerX, centerY):
    ...
    if len(contours) >= 2:
        ...
        for i in range(len(biggestCnts) - 1):
            ...
            if (np.sign(tilt1) != np.sign(tilt2)):
                ...
                # Merge all rectangle points into a new array
                pts = merge(biggestCnts[i][4], biggestCnts[i + 1][4])
                #Make sure no duplicates, then append
                if not targets:
                    targets.append([centerOfTarget, yawToTarget, pts])
    ...
    if (len(targets) > 0):
        ...
        # success, angles, translation_vector = calculateAnglesAndDistances(finalTarget[2])
        # Puts the yaw on screen
        #Draws yaw of target + line where center of target is
        ...
        # currentHorizontalDistanceError = finalTarget[2]
        # pushes vision target angle to network tables
        networkTable.putNumber("tapeYaw", currentAngleError)
        # networkTable.putNumber("tapeXDistance", currentHorizontalDistanceError)

        # networkTable.putNumber("angle1", angles[0])
        # networkTable.putNumber("angle2", angles[1])
        # networkTable.putNumber("angle3", angles[2])

        # networkTable.putNumber("distance1", translation_vector[0])
        # networkTable.putNumber("distance2", translation_vector[1])
        # networkTable.putNumber("distance3", translation_vector[2])
    else:
